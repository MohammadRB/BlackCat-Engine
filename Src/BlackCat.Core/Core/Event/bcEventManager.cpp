...
namespace black_cat
{
	namespace core
	{
		...
		bcUINT32 bc_event_manager::process_event_queue(const core_platform::bc_clock::update_param& p_clock_update_param)
		{
			...
			while (!m_local_queue.empty() && m_local_queue.back().m_process_time <= p_clock_update_param.m_total_elapsed)
			{
			...
			m_total_elapsed = p_clock_update_param.m_total_elapsed;
