...
namespace black_cat
{
	namespace editor
	{
		bc_terrain_material_ui_command::bc_terrain_material_ui_command(bcUINT16 p_screen_width,
		...
		bc_terrain_material_ui_command::bc_terrain_material_ui_command(const bc_terrain_material_ui_command&) = default;

		bc_terrain_material_ui_command::~bc_terrain_material_ui_command() = default;

		bc_terrain_material_ui_command& bc_terrain_material_ui_command::operator=(const bc_terrain_material_ui_command&) = default;

		core::bc_string bc_terrain_material_ui_command::title() const
		{
			return "TerrainMaterial";
		}

		bci_ui_command::state_ptr bc_terrain_material_ui_command::create_state(state_context& p_context) const
		{
			...
			auto l_cb_config = graphic::bc_graphic_resource_builder().as_resource()
				...
				(
					1,
					sizeof(bc_terrain_material_ui_command_parameter_cbuffer),
					graphic::bc_resource_usage::gpu_rw,
					graphic::bc_resource_view_type::none
				)
				.as_constant_buffer();

			bc_terrain_material_ui_command_state l_state;
			...
			l_state.m_parameter_cbuffer->set_debug_name("terrain_material_cbuffer");

			return core::bc_make_unique<bc_terrain_material_ui_command_state>(std::move(l_state));
		}

		bool bc_terrain_material_ui_command::update(terrain_update_context& p_context)
		{
			...
			bc_terrain_material_ui_command_parameter_cbuffer l_cbuffer_parameters;
			...
			bc_terrain_material_ui_command_render_task l_render_task
			(
				l_dx11_height_map,
				*static_cast<bc_terrain_material_ui_command_state*>(p_context.m_state),
		...
		bc_terrain_material_ui_command_render_task::bc_terrain_material_ui_command_render_task(const bc_editor_height_map_dx11& p_height_map,
			bc_terrain_material_ui_command_state& p_command_state,
			const bc_terrain_material_ui_command_parameter_cbuffer& p_shader_parameter)
		...
		void bc_terrain_material_ui_command_render_task::execute(game::bc_render_system& p_render_system, game::bc_render_thread& p_render_thread)
		{
