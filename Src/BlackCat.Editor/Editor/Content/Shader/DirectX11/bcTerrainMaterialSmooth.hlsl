...
{
    return g_texturemap_srv[p_texcoord];
}
    ...
    uint3 p_dispatch_thread_id : SV_DispatchThreadID)
{
    ...
    uint l_min_x = max(0, l_global_coord.x - g_tool_smooth);
    uint l_min_z = max(0, l_global_coord.y - g_tool_smooth);
    uint l_max_x = min(g_width, l_global_coord.x + g_tool_smooth);
    uint l_max_z = min(g_height, l_global_coord.y + g_tool_smooth);
    uint l_num_sample = 0;
    ...
    for (uint i = 0; i < 8; ++i)
    {
        float l_multiplier = l_unpacked_values[i] / 255.0;
        uint l_normalization = l_multiplier * l_diff;
