...
namespace black_cat
{
	namespace editor
	{
		...
		{
			m_selected_actor = std::move(p_actor);

			if (m_selected_actor.is_valid())
			{
				const auto* l_mediate_component = m_selected_actor.get_component<game::bc_mediate_component>();
				const auto l_position = p_transform.get_translation();
				const auto l_euler_rotation = bc_matrix4f_decompose_to_angles_zyx(p_transform);
		...
		void bc_form_object::actorUpdateClick(bool)
		{
			...
			l_transform.set_rotation(bc_matrix3f_rotation_zyx(l_rotation));
