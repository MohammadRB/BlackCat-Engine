	...
	namespace editor
	{
		...
		void bc_form_object::setSelectedActor(game::bc_actor p_actor, const core::bc_matrix4f& p_transform)
		{
			...
			if (m_selected_actor.is_valid())
			{
				...
				m_entity_up_x_text->setText(QString::number(l_up.x));
				m_entity_up_x_text->setEnabled(true);
				m_entity_up_y_text->setText(QString::number(l_up.y));
				m_entity_up_y_text->setEnabled(true);
				m_entity_up_z_text->setText(QString::number(l_up.z));
				m_entity_up_z_text->setEnabled(true);
				m_entity_up_r_text->setText(QString::number(l_up_rotation));
		...
		void bc_form_object::actorUpdateClick(bool)
		{
			...
			l_rotation.rotation_euler_lh(l_up, core::bc_to_radian(l_up_rotation));
