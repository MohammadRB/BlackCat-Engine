...
namespace black_cat::editor
{
	void bc_editor_height_map_loader_dx11::content_processing(core::bc_content_loading_context& p_context) const
	{
		...
		auto l_loaded_height_map = p_context.m_result->release_result<bc_height_map_dx11>();
		auto l_height_map = l_loaded_height_map->get_height_map();
		auto l_texture_map = l_loaded_height_map->get_texture_map();

		auto l_height_map_unordered_view_config = graphic::bc_graphic_resource_builder()
			.as_resource_view()
			.as_texture_view(l_height_map.get_format())
			.as_tex2d_unordered_shader_view(0)
			.on_texture2d();
		auto l_texture_map_unordered_view_config = graphic::bc_graphic_resource_builder()
			.as_resource_view()
	...
	std::pair<graphic::bc_texture_config, graphic::bc_resource_view_config> bc_editor_height_map_loader_dx11::get_texture_map_texture_config(bcUINT32 p_width, 
		bcUINT32 p_height) const
	{
		auto l_texture_config = graphic::bc_graphic_resource_builder()
			.as_resource()
