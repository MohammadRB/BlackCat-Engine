...
namespace black_cat
{
	namespace graphic
	{
		...
		public:
			...
			explicit bc_render_target_view_parameter(const bc_shader_parameter_link& p_parameter_link);
		...
		inline bc_render_target_view_parameter::bc_render_target_view_parameter(const bc_shader_parameter_link& p_parameter_link)
			: m_render_target_view(),
			m_link(&p_parameter_link)
		{
		}

		inline bc_render_target_view bc_render_target_view_parameter::get_render_target_view() const
		{
			return m_render_target_view.is_valid()
				       ? m_render_target_view
				       : m_link
					         ? m_link->get_as_render_target_view()
