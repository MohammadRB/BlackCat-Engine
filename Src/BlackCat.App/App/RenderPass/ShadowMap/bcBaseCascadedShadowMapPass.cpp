...
namespace black_cat
{
	...
	void bc_base_cascaded_shadow_map_pass::execute(const game::bc_concurrent_render_pass_render_context& p_context)
	{
		...
		for (bcSIZE l_light_ite = 0; l_light_ite < m_state->m_lights.size(); ++l_light_ite)
		{
			...
			{
				m_state->m_captured_camera = p_context.m_render_camera.get_extend_points();

				for (auto& l_cascade_camera : m_last_render_cascade_cameras)
				{
					game::bci_camera::extend_points l_extends;
		...
		const game::bc_direct_light& p_light)
	{
		...
		for (bcUINT l_cascade_index = 0; l_cascade_index < l_cascade_break_points.size() - 1; ++l_cascade_index)
		{
			...
			game::bci_camera::extend_points l_frustum_points
			{
			...
			game::bci_camera::extend_points l_frustum_points_vs
			{
