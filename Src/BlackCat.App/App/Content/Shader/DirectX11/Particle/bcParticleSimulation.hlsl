...
float3 sample_curve(uint3 p_curve_indexes, float p_time)
{
	...
	float3 l_curve_samples = l_normalized_time_between_samples * (l_upper_band_samples - l_lower_band_samples) + l_lower_band_samples;
...
{
	const uint l_particle_index = p_dispatch_thread_id.x;
	...
	const float3 l_velocity_curve_sample = sample_curve
	(
	...
	const float3 l_side_fade_curve_sample = sample_curve
	(
	...
	const float l_velocity_curve_value = l_velocity_curve_sample.x;
	const float l_size_curve_value = 1 - l_side_fade_curve_sample.x;
	const float l_fade_curve_value = l_side_fade_curve_sample.y;
	const float l_acceleration = l_particle.m_force * l_velocity_curve_value;
	const float l_wind_influence = max(0.1, (1 - saturate(l_particle.m_mass / 0.3f)));
	...
	const uint l_alive_index = g_alive_indices.IncrementCounter();
