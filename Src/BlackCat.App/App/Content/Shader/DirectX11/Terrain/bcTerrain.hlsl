...
{
	const float l_scale = g_chunk_size / g_scale;
	const float l_max_dis = distance(float2(0, 0), float2(g_width * g_xz_multiplier, g_height * g_xz_multiplier));
	const float l_min_dis = (g_chunk_size * g_xz_multiplier) / 2;
	const float l_distance = distance(p_patch_center, g_camera_position) - g_distance_detail;

	const float l_height_tess_factor = p_patch_diff_height * g_height_detail;
	float l_tess_factor = 1 - saturate((l_distance - l_min_dis) / (l_max_dis - l_min_dis));
	l_tess_factor = pow(l_tess_factor, 16);
	l_tess_factor *= l_scale;
	l_tess_factor += l_height_tess_factor;

	return min(l_scale, max(l_tess_factor, 2));
}
...
bc_constant_hs_output constant_hs(InputPatch<bc_vs_output, 4> p_input, uint p_patch_id : SV_PrimitiveID)
{
	...
	const float3 l_min_vertex = float3(p_input[0].m_position.x, l_patch_info.x, p_input[0].m_position.z);
	const float3 l_max_vertex = float3(p_input[2].m_position.x, l_patch_info.y, p_input[2].m_position.z);

	const float3 l_box_center = 0.5f * (l_min_vertex + l_max_vertex);
	const float3 l_box_extents = 0.5f * (l_max_vertex - l_min_vertex);
	...
	const uint l_width_chunk_count = g_width / g_chunk_size;
	const uint l_height_chunk_count = g_height / g_chunk_size;
	const uint l_chunk_size = g_chunk_size * g_xz_multiplier;
	...
	const uint l_left_patch_id = l_left_patch_index.y * l_width_chunk_count + l_left_patch_index.x;
	const uint l_right_patch_id = l_right_patch_index.y * l_width_chunk_count + l_right_patch_index.x;
	const uint l_top_patch_id = l_top_patch_index.y * l_width_chunk_count + l_top_patch_index.x;
	const uint l_bottom_patch_id = l_bottom_patch_index.y * l_width_chunk_count + l_bottom_patch_index.x;

	float3 l_left_patch_info = g_chunk_info[l_left_patch_id];
	float3 l_right_patch_info = g_chunk_info[l_right_patch_id];
	float3 l_top_patch_info = g_chunk_info[l_top_patch_id];
	float3 l_bottom_patch_info = g_chunk_info[l_bottom_patch_id];
	...
	const int l_patch_height_diff = l_patch_info.z;
	const int l_left_patch_height_diff = l_left_patch_info.z;
	const int l_right_patch_height_diff = l_right_patch_info.z;
	const int l_top_patch_height_diff = l_top_patch_info.z;
	const int l_bottom_patch_height_diff = l_bottom_patch_info.z;

	const float l_patch_tess_factor = calc_tess_factor(l_patch_center, l_patch_height_diff);
	const float l_left_patch_tess_factor = calc_tess_factor(l_left_patch_center, l_left_patch_height_diff);
	const float l_right_patch_tess_factor = calc_tess_factor(l_right_patch_center, l_right_patch_height_diff);
	const float l_top_patch_tess_factor = calc_tess_factor(l_top_patch_center, l_top_patch_height_diff);
	const float l_bottom_patch_tess_factor = calc_tess_factor(l_bottom_patch_center, l_bottom_patch_height_diff);
...
{
	bc_ds_output l_output;

	const float3 l_vertical_pos1 = lerp(p_quad[0].m_position, p_quad[1].m_position, UV.y);
	const float3 l_vertical_pos2 = lerp(p_quad[3].m_position, p_quad[2].m_position, UV.y);
...
bc_ps_gbuffer_output gbuffer_ps(bc_ds_output p_input)
{
	...
	l_output.m_specular = float4(l_specular_intensity, l_specular_power / g_specular_power_scale, 0, 1);
