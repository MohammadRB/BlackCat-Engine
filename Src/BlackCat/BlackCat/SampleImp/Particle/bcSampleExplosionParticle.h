...
namespace black_cat
{
	class bc_sample_explosion_particle
	{
	public:
		game::bc_particle_builder operator()(core::bc_random& p_random) const;
	};

	inline game::bc_particle_builder bc_sample_explosion_particle::operator()(core::bc_random& p_random) const
	{
		core::bc_array< core::bc_vector3f, 30 > l_big_explosion_random_directions;
		bc_randomize_direction
		(
			p_random,
			core::bc_vector3f::up(),
			180,
			std::begin(l_big_explosion_random_directions),
			std::end(l_big_explosion_random_directions)
		);

		game::bc_particle_builder l_big_explosion_builder;
		l_big_explosion_builder.emitter(core::bc_vector3f(0), core::bc_vector3f::up())
		...
		l_big_explosion_builder.emitter(core::bc_vector3f(0, -8, 0), core::bc_vector3f::up())
		...
		for (auto& l_direction : l_big_explosion_random_directions)
		{
			const auto l_position = core::bc_vector3f(l_direction.x, 0, l_direction.z) * 6;
			const auto l_emitter_energy = l_direction.y;

			l_big_explosion_builder.emitter(l_position, l_direction, 0.2f, 1500 * l_emitter_energy, 0.1f)
		...
		return l_big_explosion_builder;
	}
}