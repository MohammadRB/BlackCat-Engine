	...
	namespace graphic
	{
		...
		void bc_platform_compute_stage<g_api_dx11>::apply_shader_resource_views(bc_device_pipeline& p_pipeline)
		{
			...
			if (m_required_state.m_unordered_access_views.update_needed())
			{
				...
				const bcUINT32 l_dirty_slot_start = m_required_state.m_unordered_access_views.get_dirty_start();
				const bcUINT32 l_dirty_slot_num = m_required_state.m_unordered_access_views.get_dirty_count();
				const bcUINT32 l_dirty_slot_end = l_dirty_slot_start + l_dirty_slot_num;
