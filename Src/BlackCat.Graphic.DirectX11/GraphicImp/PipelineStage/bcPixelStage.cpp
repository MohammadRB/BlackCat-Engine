...
namespace black_cat::graphic
{
	...
	void bc_platform_pixel_stage<g_api_dx11>::apply_constant_buffers(bc_device_pipeline& p_pipeline)
	{
		...
		if (m_required_state.m_constant_buffers.update_needed())
		{
			...
			for (auto l_i = l_dirty_slot_start; l_i < l_dirty_slot_end; ++l_i)
			{
				bc_buffer l_buffer = m_required_state.m_constant_buffers.get(l_i);
				l_constant_buffers[l_i] = l_buffer.is_valid() ? l_buffer.get_platform_pack().m_buffer : nullptr;
			}
	...
	void bc_platform_pixel_stage<g_api_dx11>::apply_sampler_states(bc_device_pipeline& p_pipeline)
	{
		...
		if (m_required_state.m_sampler_states.update_needed())
		{
			...
			for (auto l_i = l_dirty_slot_start; l_i < l_dirty_slot_end; ++l_i)
			{
				bc_sampler_state l_sampler_state = m_required_state.m_sampler_states.get(l_i);
				l_sampler_states[l_i] = l_sampler_state.is_valid() ? l_sampler_state.get_platform_pack().m_sampler_state : nullptr;
			}
	...
	void bc_platform_pixel_stage<g_api_dx11>::apply_shader_resource_views(bc_device_pipeline& p_pipeline)
	{
		...
		if (m_required_state.m_shader_resource_views.update_needed())
		{
			...
			for (auto l_i = l_dirty_slot_start; l_i < l_dirty_slot_end; ++l_i)
			{
				bc_resource_view l_shader_view = m_required_state.m_shader_resource_views.get(l_i);
				l_views[l_i] = l_shader_view.is_valid() ? l_shader_view.get_platform_pack().m_shader_view : nullptr;
			}
