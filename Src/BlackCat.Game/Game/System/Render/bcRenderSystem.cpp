...
namespace black_cat
{
	namespace game
	{
		...
		{
			m_frame_renderer->update(bc_frame_renderer_update_param(p_update_params.m_clock, p_update_params.m_active_camera));
		}

		void bc_render_system::render(const render_param& p_render_param)
		{
			m_frame_renderer->render(bc_frame_renderer_render_param(p_render_param.m_clock, *this, p_render_param.m_camera, p_render_param.m_scene));
