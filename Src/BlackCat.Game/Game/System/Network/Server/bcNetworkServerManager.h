...
namespace black_cat
{
	namespace game
	{
		...
		class BC_GAME_DLL bc_network_server_manager : public bci_network_manager, private bc_server_socket_state_machine
		{
		public:
			...
			void client_disconnected(bc_network_server_manager_client& p_client);
		...
		private:
			...
			void on_enter(bc_server_socket_accepting_state& p_state) override;

			void _send_to_client(const core_platform::bc_clock::update_param& p_clock, bc_network_server_manager_client& p_client);

			void _receive_from_client(const core_platform::bc_clock::update_param& p_clock, bc_network_server_manager_client& p_client);
