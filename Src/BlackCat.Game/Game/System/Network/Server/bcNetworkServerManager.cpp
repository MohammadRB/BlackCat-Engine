...
namespace black_cat
{
	namespace game
	{
		...
		{
			{
				...
				{
					core_platform::bc_lock_guard<bc_network_server_manager_client1> l_client_lock(l_client);
		...
		{
			m_clients.push_back(bc_network_server_manager_client1(p_address));
		...
		void bc_network_server_manager::_retry_messages_with_acknowledgment(bc_network_packet_time p_current_time, bc_network_server_manager_client1& p_client)
		{
		...
		void bc_network_server_manager::_send_to_client(bc_network_server_manager_client1& p_client)
		{
			...
			{
				core_platform::bc_lock_guard<bc_network_server_manager_client1> l_lock(p_client);
				...
				bc_server_socket1_send_event l_send_event{p_client.get_address(), *l_stream, l_stream_size};
		...
		bc_network_server_manager_client1* bc_network_server_manager::_find_client(const platform::bc_network_address& p_address)
		{
			const auto l_ite = std::find_if
			(
				std::begin(m_clients),
