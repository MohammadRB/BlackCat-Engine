	...
	namespace game
	{
		...
		void bc_network_client_manager::remove_actor(bc_actor& p_actor)
		{
...

			{
				...
				const auto l_ite = std::find(std::cbegin(m_new_actors), std::cend(m_new_actors), p_actor);
				if (l_ite != std::end(m_new_actors))
				{
					m_new_actors.erase(l_ite);
				}
			}
		}
		...
		void bc_network_client_manager::on_enter(const bc_client_socket_error_state& p_state)
		{
		...
		void bc_network_client_manager::on_enter(const bc_client_socket_connecting_state& p_state)
		{
			m_hook->connecting_to_server(m_ip, m_port);
		}

		void bc_network_client_manager::on_enter(const bc_client_socket_connected_state& p_state)
		{
		...
		void bc_network_client_manager::on_enter(const bc_client_socket_sending_state& p_state)
		{
			m_socket_is_ready = false;
		}
		...
		{
			m_receive_buffer.set_position(core::bc_stream_seek::start, 0);
			
			bc_client_socket_receive_event l_receive_event{ m_receive_buffer, 0 };
			...
			for(auto& l_message : l_deserialized_messages.second)
			{
