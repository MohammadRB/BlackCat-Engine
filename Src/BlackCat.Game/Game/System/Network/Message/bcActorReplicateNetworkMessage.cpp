...
namespace black_cat
{
	namespace game
	{
		...
		bc_actor_replicate_network_message::bc_actor_replicate_network_message(const bc_actor& p_actor_to_serialize)
			: bci_network_message(message_name()),
			m_actor_to_serialize(p_actor_to_serialize)
		{
		}
		...
		void bc_actor_replicate_network_message::serialize_message(const bc_network_message_serialization_context& p_context)
		{
			...
			const auto* l_mediate_component = m_actor_to_serialize.get_component<bc_mediate_component>();
			...
			const bc_actor_component_network_write_context l_context(p_context.m_params, m_actor_to_serialize);
		...
		void bc_actor_replicate_network_message::deserialize_message(const bc_network_message_deserialization_context& p_context)
		{
			...
			if(!l_entity_name)
			{
				core::bc_log(core::bc_log_type::error, bcL("Failed to deserialize actor entity name in replicated network message"));
				return;
			}
