	...
	namespace game
	{
		...
		void bc_light_component::initialize(bc_actor& p_actor, const core::bc_data_driven_parameter& p_parameters)
		{
			...
			if(l_light_type == "direct")
			{
				...
				const auto l_ambient_color = p_parameters.get_value_throw< core::bc_vector3f >("ambient_color");
				const auto l_ambient_intensity = p_parameters.get_value_throw< bcFLOAT >("ambient_intensity");

				m_light = l_light_manager.add_light(bc_direct_light(l_direction, l_color, l_intensity, l_ambient_color, l_ambient_intensity));
			}
			else if(l_light_type == "point")
			{
				...
				m_light = l_light_manager.add_light(bc_point_light(l_position, l_radius, l_color, l_intensity));
			}
			else if(l_light_type == "spot")
			{
				...
				m_light = l_light_manager.add_light(bc_spot_light(l_position, l_direction, l_length, l_angle, l_color, l_intensity));
			}
		}
