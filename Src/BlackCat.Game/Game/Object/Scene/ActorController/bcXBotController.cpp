...
namespace black_cat
{
	namespace game
	{
		...
		void bc_xbot_controller::handle_event(const bc_actor_component_event_context& p_context)
		{
			...
			{
				bc_animation_job_helper::set_skinning_world_transform(static_cast<bc_sequence_animation_job&>(*m_idle_job), l_world_transform_event->get_transform());
				bc_animation_job_helper::set_skinning_world_transform(static_cast<bc_sequence_animation_job&>(*m_running_job), l_world_transform_event->get_transform());
			}
		}
		...
		void bc_xbot_controller::_create_running_animation(bc_actor& p_actor)
		{
			...
			auto l_blending_job = core::bc_make_shared<bc_blending_animation_job>
			(
				bc_blending_animation_job(l_skeleton, { m_idle_sample_job, m_running_sample_job, m_running_backward_sample_job })
			);
			auto l_local_to_model_job = core::bc_make_shared<bc_local_to_model_animation_job>
			(
				bc_local_to_model_animation_job(l_blending_job, m_skinned_component->get_mesh(), m_skinned_component->get_model_transforms())
