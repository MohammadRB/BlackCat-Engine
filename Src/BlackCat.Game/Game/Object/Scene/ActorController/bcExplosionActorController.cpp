...
namespace black_cat
{
	namespace game
	{
		...
		void bc_explosion_actor_controller::added_to_scene(const bc_actor_component_event_context& p_context, bc_scene& p_scene)
		{
			m_scene = &p_scene;
		...
		void bc_explosion_actor_controller::update(const bc_actor_component_update_content& p_context)
		{
			if(m_scene_terrain_query.is_executed())
			{
				auto l_hit_result = *m_scene_terrain_query.get().get_result().as<core::bc_nullable<bc_ray_hit>>();

				auto* l_mediate_component = p_context.m_actor.get_component< bc_mediate_component >();
