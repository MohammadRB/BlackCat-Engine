...
namespace black_cat
{
	namespace core
	{
		...
		{
			_register_listener(p_type, _bc_log_listener_container(std::move(p_listener)));
		}

		void bc_logger::register_listener(bc_log_type p_type, bc_ilog_listener* p_listener)
		{
			_register_listener(p_type, _bc_log_listener_container(p_listener));
		}

		void bc_logger::_register_listener(bc_log_type p_type, _bc_log_listener_container&& p_listener)
		{
			bc_array<bc_log_type, 3> l_types;
			...
			for (auto l_type : l_types)
			{
				...
				{
					bc_vector< _bc_log_listener_container > l_listeners;
