...
namespace black_cat
{
	namespace core
	{
		...
		protected:
			...
			bcInline void _move_elements(node_type* p_dest, node_type* p_src, size_type p_count)
			{
				for (bcUINT32 l_i = 0; l_i < p_count; ++l_i)
				{
					...
					{
						bc_allocator_traits< internal_allocator_type >::destruct(
							m_allocator,
							p_src + l_i);
					}
				}
			...
			bcInline void _move_if_noexcept_elements(node_type* p_dest, node_type* p_src, size_type p_count)
			{
				for (bcUINT32 l_i = 0; l_i < p_count; ++l_i)
				{
					bc_allocator_traits<internal_allocator_type>::construct(
			...
			bcInline void _change_capacity(size_type p_new_capacity)
			{
				...
				if (m_first)
				{
					try
					{
						_move_if_noexcept_elements(l_heap, m_first, std::min< size_type >(base_type::m_size, p_new_capacity));
					}
					catch (...)
					{
			...
			bcInline void _increase_capacity(size_type p_count_to_add = 0)
			{
				...
				size_type l_new_capacity = m_capacity == 0 ? 1 : m_capacity;

				while (p_count_to_add > l_reserved_count)
				{
					l_new_capacity *= 2;
					l_reserved_count = (l_new_capacity - base_type::m_size);
				}

				if (l_new_capacity <= m_capacity)
					return;
			...
			node_type* _new_node(node_type* p_position, TInputIterator p_first, TInputIterator p_last, std::forward_iterator_tag&)
			{
				...
				std::for_each(p_first, p_last, [=, &l_position](typename std::iterator_traits<TInputIterator>::value_type& p_value)->void
				{
			...
			node_type* _free_node(node_type* p_position, size_type p_count)
			{
				...
				size_type l_count = p_count;
				while (l_count-- > 0)
					bc_allocator_traits< internal_allocator_type >::destruct(m_allocator, p_position + l_count);

				if (p_position != _get_end() - 1)
				{
		...
		private:
			...
			bcInline node_type& _get_node(size_type p_position)
			{
				bcAssert(p_position >= 0 && p_position <= base_type::m_size && base_type::m_size > 0);
		...
		void bc_vector<T, TAllocator>::assign(std::initializer_list<value_type> p_initializer)
		{
			...
			base_type::_new_node(base_type::m_first,
				std::begin(p_initializer),
				std::end(p_initializer),
				typename std::iterator_traits< std::initializer_list<value_type> >::iterator_category());
		}
