...
namespace box
{
	void bx_fire_actor_controller::initialize(const game::bc_actor_component_initialize_context& p_context)
	{
		...
		const auto l_emitter = game::bc_particle_builder()
			...
			.with_texture(4)
			.with_particles_color({ 0.2f, 0.2f, 0.2f }, 1)
			...
			.emit_particles_with_per_second_count(25, 10, 20, -0.1f)
			.duplicate_last(core::bc_vector3f(0), core::bc_vector3f::up())
			.with_texture(0)
			.emit_particles_with_per_second_count(25, 11, 19, -0.1f)
			.duplicate_last(core::bc_vector3f(0), core::bc_vector3f::up())
			.with_texture(10)
			.with_particles_color({ 0.9f, 0.005f, 0.001f }, 2.5)
