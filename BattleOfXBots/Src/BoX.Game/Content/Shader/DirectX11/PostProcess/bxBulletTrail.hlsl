...
{
	float m_length							: packoffset(c0.x);
	float m_width							: packoffset(c0.y);
}
...
void bullet_trail_gs(point bc_vs_output p_input[1], inout TriangleStream<bc_gs_output> p_stream)
{
	...
	for (uint i = 0; i < 4; ++i)
	{
		...
		l_quad *= float2(m_length, m_width);
...
{
	const float4 l_mask = g_texture.Sample(g_linear_sampler, p_input.m_texcoord);
	return l_mask;
}