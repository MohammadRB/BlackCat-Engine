...
namespace black_cat
{
	namespace graphic
	{
		...
		template< bc_platform_render_api TRenderApi >
		class bc_platform_shader_view : public bc_iresource_view
		{
		public:
			using platform_pack = bc_platform_shader_view_pack<TRenderApi>;

		public:
			bc_platform_shader_view();

			bc_platform_shader_view(bc_platform_shader_view&&);

			~bc_platform_shader_view();

			bc_platform_shader_view& operator=(bc_platform_shader_view&&);
		...
		using bc_shader_view_ptr = bc_resource_ptr< bc_shader_view >;
	}
}
