...
namespace black_cat
{
	...
	{
		template< >
		inline const bcCHAR* bc_device_resource_content<bc_domain_shader>::content_name()
		{
			return core::g_cnt_domain_shader;
		}

		/*struct bc_domain_shader_loader_parameter
		{
			bc_domain_shader_loader_parameter(bc_device& p_device, const bcCHAR* p_function)
				: m_device(p_device),
		...
		class BC_GRAPHICSHARED_DLL bc_domain_shader_loader : public core::bc_base_content_loader
		{
		public:
			...
			bc_domain_shader_loader& operator=(bc_domain_shader_loader&&);
