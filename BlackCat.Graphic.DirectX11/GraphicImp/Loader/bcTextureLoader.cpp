...
#include "GraphicImp/Loader/bcTextureLoader.h"
...
namespace black_cat
{
	namespace graphic
	{
		...
		void bc_texture_loader::content_processing(core::bc_content_loader_context& p_context)
		{
			...
			bc_texture_config l_config = bc_graphic_resource_configure().as_resource().as_texture2d
				(
					0,
					0,
					false, // DX11 requires shader_bind_flag & render_target_bind_flag to be set for auto mip generation
					0,
					bc_format::R8G8B8A8_UNORM,
					bc_resource_usage::gpu_r,
					bc_resource_view_type::shader
				)
				.as_normal_texture();

			bc_texture2d_ptr l_result = l_device->create_texture2d
				(
					l_config,
			...
			p_context.set_result(bc_texture2d_content(std::move(l_result)));
		}
	}
}