...
#include "GraphicImp/Loader/bcDomainShaderLoader.h"
...
namespace black_cat
{
	namespace graphic
	{
		...
		bc_domain_shader_loader::bc_domain_shader_loader(bc_domain_shader_loader&& p_other)
		...
		bc_domain_shader_loader& bc_domain_shader_loader::operator=(bc_domain_shader_loader&& p_other)
		{
		...
		void bc_domain_shader_loader::content_offline_processing(core::bc_content_loader_context& p_context)
		{
			...
			bc_compiled_shader_ptr l_compiled_shader = l_device->compile_domain_shader
			(
		...
		void bc_domain_shader_loader::content_processing(core::bc_content_loader_context& p_context)
		{
			bc_device* l_device = p_context.m_parameter.get_value_throw< bc_device* >(core::g_param_device);
			core::bc_string& l_function = p_context.m_parameter.get_value_throw< core::bc_string >(core::g_param_shader_function);

			bc_domain_shader_ptr l_result = l_device->create_domain_shader
				(
					p_context.m_data.data(), 
					p_context.m_data.size(),
					l_function.c_str()
			...
			p_context.set_result(bc_domain_shader_content(std::move(l_result)));
		}
	}
}
