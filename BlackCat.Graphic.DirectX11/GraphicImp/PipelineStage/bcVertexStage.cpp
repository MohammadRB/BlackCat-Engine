...
namespace black_cat
{
	namespace graphic
	{
		...
		template<>
		BC_GRAPHICIMP_DLL_EXP
		bc_platform_vertex_stage< g_api_dx11 >::bc_platform_vertex_stage(bc_platform_vertex_stage&& p_other)
		...
		template<>
		BC_GRAPHICIMP_DLL_EXP
		bc_platform_vertex_stage< g_api_dx11 >& bc_platform_vertex_stage< g_api_dx11 >::operator=(bc_platform_vertex_stage&& p_other)
		{
		...
		void bc_platform_vertex_stage< g_api_dx11 >::apply_constant_buffers(bc_device_pipeline* p_pipeline)
		{
			...
			if (m_required_state.m_constant_buffers.update_needed())
			{
				...
				l_context->VSSetConstantBuffers(l_dirty_slot_start, l_dirty_slot_num, l_constant_buffers);
			}
		};
