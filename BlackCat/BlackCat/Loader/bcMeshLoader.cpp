...
namespace black_cat
{
	bcSIZE _calculate_node_count(aiNode* p_begin);

	void _convert_aimatrix(const aiMatrix4x4& p_aimatrix, graphic::bc_matrix4f& p_matrix);

	void _convert_aimaterial(core::bc_content_loader_context& p_context, const aiMaterial& p_aimaterial, game::bc_mesh_node_material& p_material);

	void _convert_ainodes_to_mesh_nodes(core::bc_content_loader_context& p_context, const aiScene* p_aiscene, const aiNode* p_ainode, game::bc_mesh* p_mesh, game::bc_mesh_node* p_parent);

	bc_mesh_loader::bc_mesh_loader()
	{
	}

	bc_mesh_loader::bc_mesh_loader(bc_mesh_loader&& p_other)
	...
	bc_mesh_loader& bc_mesh_loader::operator=(bc_mesh_loader&& p_other)
	{
	...
	void bc_mesh_loader::content_processing(core::bc_content_loader_context& p_context)
	{
		...
		{
			p_context.set_result(bc_io_exception("Content file doesn't include mesh data"));
		}

		auto* l_game_system = core::bc_service_manager::get().get_service< game::bc_game_system >();
		// Set device to parameters because we use texture loader
		p_context.m_parameter.add_value(core::g_param_device, &l_game_system->get_render_system().get_device());

		bcSIZE l_node_count = _calculate_node_count(l_scene->mRootNode);

		core::bc_estring l_mesh_name = core::bc_path(p_context.m_file_path.c_str()).get_filename();
		game::bc_mesh l_mesh(core::bc_to_exclusive_string(l_mesh_name), l_node_count);

		_convert_ainodes_to_mesh_nodes(p_context, l_scene, l_scene->mRootNode, &l_mesh, nullptr);
	...
	bcSIZE _calculate_node_count(aiNode* p_begin)
	{
		...
		{
			l_count += _calculate_node_count(p_begin->mChildren[i]);
		}
	...
	void _convert_ainodes_to_mesh_nodes(core::bc_content_loader_context& p_context, const aiScene* p_aiscene, const aiNode* p_ainode, game::bc_mesh* p_mesh, game::bc_mesh_node* p_parent)
	{
		...
		aiMesh* l_node_mesh = p_ainode->mNumMeshes > 0 ? p_aiscene->mMeshes[p_ainode->mMeshes[0]] : nullptr;
		...
		graphic::bc_shader_view_ptr l_diffuse_map_view;
		graphic::bc_shader_view_ptr l_specular_map_view;
		graphic::bc_shader_view_ptr l_normal_map_view;
		game::bc_mesh_node_material l_material;
		game::bc_render_state_ptr l_render_state;

		if (l_node_mesh)
		{
			bool l_need_32bit_indices = l_node_mesh->mNumFaces * 3 > std::numeric_limits< bcUINT16 >::max();
			bool l_has_texcoord = l_node_mesh->HasTextureCoords(0);
			bool l_has_normal = l_node_mesh->HasNormals();
			bool l_has_tangent = l_node_mesh->HasTangentsAndBitangents();

			l_vertices.reserve(l_node_mesh->mNumVertices);
			l_indices.reserve(l_node_mesh->mNumFaces * 3 * (l_need_32bit_indices ? static_cast< bcINT >(game::bc_index_type::i32bit) : static_cast< bcINT >(game::bc_index_type::i16bit)));

			for (bcUINT l_vertex = 0; l_vertex < l_node_mesh->mNumVertices; ++l_vertex)
			{
				game::bc_vertex_pos_tex_nor_tan l_vertex_layout;
				auto* l_aivertex = &l_node_mesh->mVertices[l_vertex];
				auto* l_aitexcoord = l_has_texcoord ? &l_node_mesh->mTextureCoords[0][l_vertex] : nullptr;
				auto* l_ainoraml = l_has_normal ? &l_node_mesh->mNormals[l_vertex] : nullptr;
				auto* l_aitangent = l_has_tangent ? &l_node_mesh->mTangents[l_vertex] : nullptr;
			...
			for (bcUINT l_face_index = 0; l_face_index < l_node_mesh->mNumFaces; ++l_face_index)
			{
				for (bcBYTE l_index = 2; l_index >= 0; --l_index)
				{
					if (l_need_32bit_indices)
					{
						*l_32bit_indices = static_cast< bcUINT32 >(l_node_mesh->mFaces[l_face_index].mIndices[l_index]);
						++l_32bit_indices;
					}
					else
					{
						*l_16bit_indices = static_cast< bcUINT16 >(l_node_mesh->mFaces[l_face_index].mIndices[l_index]);
						++l_16bit_indices;
					}
			...
			_convert_aimaterial(p_context, *p_aiscene->mMaterials[l_node_mesh->mMaterialIndex], l_material);
			...
			l_specular_map_view = l_material.m_specular_map ?
				                      l_device->create_shader_view(l_material.m_specular_map->get_resource().get(), l_texture_view_config) :
				                      nullptr;
			l_normal_map_view = l_material.m_normal_map ?
				                    l_device->create_shader_view(l_material.m_normal_map->get_resource().get(), l_texture_view_config) :
				                    nullptr;

			l_render_state = l_game_system->get_render_system().create_render_state
				(
					...
					0,
					{
						graphic::bc_shader_view_parameter(0, graphic::bc_shader_type::pixel, l_diffuse_map_view),
						graphic::bc_shader_view_parameter(0, graphic::bc_shader_type::pixel, l_normal_map_view),
						graphic::bc_shader_view_parameter(0, graphic::bc_shader_type::pixel, l_specular_map_view),
					},
					{
		...
		for (bcUINT l_child_index = 0; l_child_index < p_ainode->mNumChildren; ++l_child_index)
		{
			...
			_convert_ainodes_to_mesh_nodes(p_context, p_aiscene, l_child, p_mesh, l_added_node);
		}
	}
}
