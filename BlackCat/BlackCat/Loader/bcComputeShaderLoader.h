...
{
	template< >
	inline constexpr bcCHAR* graphic::bc_device_resource_content< graphic::bc_compute_shader >::content_name()
	{
		return BC_CONTENT_NAME(compute_shader);
	}

	template< >
	inline constexpr bcUINT32 graphic::bc_device_resource_content< graphic::bc_compute_shader >::content_hash()
	{
		return bc_compile_time_string_hash(BC_CONTENT_NAME(compute_shader));
	}

	class BC_BLACKCAT_DLL bc_compute_shader_loader : public core::bc_base_content_loader
	{
	public:
