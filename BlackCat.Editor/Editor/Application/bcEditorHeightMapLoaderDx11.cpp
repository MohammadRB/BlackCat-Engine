...
namespace black_cat
{
	namespace editor
	{
		void bc_editor_height_map_loader_dx11::content_processing(core::bc_content_loader_context& p_context) const
		{
			...
			auto l_height_map_unordered_view_config = graphic::bc_graphic_resource_configure().as_resource_view()
				.as_texture_view(graphic::bc_format::R32_SINT)
				.as_tex2d_unordered_shader_view(0)
				.on_texture2d();

			auto l_height_unordered_view = l_render_system.get_device()
				.create_resource_view(l_height_map, l_height_map_unordered_view_config);

			p_context.set_result(bc_editor_height_map_dx11(std::move(*l_loaded_height_map.release()), l_height_unordered_view));
		}

		graphic::bc_texture_config bc_editor_height_map_loader_dx11::get_height_map_texture_config(bcUINT32 p_width, bcUINT32 p_height) const
		{
			// Because in editor we need unordered view on height map texture and DX11.0 doesn't support UAV on R16_SINT format we use R32_SINT
